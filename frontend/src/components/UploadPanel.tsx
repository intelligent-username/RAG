...
export default function UploadPanel({ onAdd }: Props) {
  ...
  const removeFile = (fname: string) => { setFiles((p) => p.filter((f) => f.name !== fname)); setVectorized(false); };
  ...
  const handleAdd = () => {
    ...
    setName(""); setFiles([]); setVectorized(false);
  };
    ...
    <>
      ...
      <div className="panel-actions">
        <button className="btn-vectorize" onClick={handleVectorize}
          ...
          {vectorizing ? "Vectorizing…" : vectorized ? "✓ Done" : "Vectorize"}
        </button>
